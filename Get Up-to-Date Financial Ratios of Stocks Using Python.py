...
apiKey = "9090426fff71f3e2b3487eb1b91e3771"
ticker = "BABA"
...
print(q_cash_flow_statement.iloc[:,4:].head())

cash_flow_statement = pd.DataFrame(
    get_jsonparsed_data(base_url + 'cash-flow-statement/' + ticker + '?apikey=' + apiKey))
...
def get_finviz_data(ticker):
    try:
        ...
        soup = bs(requests.get(url, headers={
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:20.0) Gecko/20100101 Firefox/20.0'}).content,features="lxml")
        dict_finviz = {}
        for m in metric:
...
def calculate_intrinsic_value(cash_flow, total_debt, cash_and_ST_investments,
    ...
    df = pd.DataFrame.from_dict(
        {'Year': year_list, 'Cash Flow': cash_flow_list, 'Discounted Cash Flow': cash_flow_discounted_list})
    df.index = df.Year
